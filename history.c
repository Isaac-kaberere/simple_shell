...
int read_history(info_t *info)
{
	...
	struct stat st;
	char *baf = NULL, *filename = get_history_file(info);
	...
	for (k = 0; k < fsize; k++)
		...
		{
			baf[k] = 0;
			build_history_list(info, baf + last, linecount++);
			last = k + 1;
		}
	if (last != k)
		build_history_list(info, baf + last, linecount++);
 ...
 * Return: Always 0
 */
int build_history_list(info_t *info, char *baf, int linecount)
{
	list_t *node = NULL;
